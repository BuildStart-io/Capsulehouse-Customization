...
export default function Settings() {
  ...
  const initializeAndFetchQr = useCallback(async (sessionId: string, sessionStatus: string, attempt = 1) => {
    setQrLoading(true);
    if (attempt === 1) {
      setQrCode(null);
      setQrImage(null);
    ...
    try {
      ...
      if (sessionStatus !== "connected" && attempt === 1) {
        toast({ title: "Preparing QR code...", description: "Starting the WhatsApp session. This may take up to 30 seconds." });
      }
      ...
      if (image || qr) {
        ...
        setQrCode(qr);
        setQrLoading(false);
      } else if (returnedStatus === "connected") {
        toast({ title: "WhatsApp connected", description: "This session is already linked." });
        await fetchSessions();
        setQrLoading(false);
      } else if ((returnedStatus === "connecting" || returnedStatus === "need_qr") && attempt < 15) {
        // Automatically poll if still starting, up to 15 times (~45 seconds)
        setTimeout(() => initializeAndFetchQr(sessionId, sessionStatus, attempt + 1), 3000);
      } else {
        toast({
          title: "QR code is not ready yet",
          description: "WAHA is still starting the session. Please try clicking 'Get QR Code' again.",
        });
        setQrLoading(false);
      }
    } catch (error: any) {
