...
serve(async (req) => {
  ...
  try {
    ...
    if (override && !override.includes("-capsulehouse")) {
      override = override.replace("webhook-wsender", "webhook-wsender-capsulehouse");
    }
    const webhookUrl = override || `${supabaseUrl}/functions/v1/webhook-wsender-capsulehouse`;
