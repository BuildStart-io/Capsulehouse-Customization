...
serve(async (req) => {
  ...
  try {
    ...
    const supabase = createClient(supabaseUrl, supabaseServiceKey, { db: { schema: 'capsulehouse_customization' } });
...
- Your visible reply must ALWAYS be plain, human-readable text only.`;
            ...
            if (orderError) {
              ...
              try {
                ...
                if (ownerPhone) {
                  ...
                  const sendNotif = await fetch(`${supabaseUrl}/functions/v1/send-whatsapp-capsulehouse`, {
                    method: "POST",
                    headers: {
    ...
    const customerJsonMatches = [...responseText.matchAll(/<CUSTOMER_JSON>([\s\S]*?)<\/CUSTOMER_JSON>/g)];
    for (const customerMatch of customerJsonMatches) {
      try {
        const customerData = JSON.parse(customerMatch[1]);
        console.log("Saving customer lead:", JSON.stringify(customerData));
        ...
        if (existingCustomers && existingCustomers.length > 0) {
          console.log("Updating existing customer lead:", existingCustomers[0].id);
          ...
          console.log("Inserting new customer lead");
    ...
    const cancelOrderMatch = responseText.match(/<CANCEL_ORDER>true<\/CANCEL_ORDER>/i);
    if (cancelOrderMatch) {
      console.log("Processing order cancellation request for", phoneNumber);
      
      const { data: latestPendingOrders } = await supabase
        .from("orders")
        .select("id")
        .eq("user_id", userId)
      ...
      if (latestPendingOrders && latestPendingOrders.length > 0) {
        console.log("Cancelling order ID:", latestPendingOrders[0].id);
        const { error: cancelError } = await supabase
          .from("orders")
          .update({ status: "cancelled" })
          .eq("id", latestPendingOrders[0].id);
        
        if (cancelError) {
          console.error("Error cancelling order:", cancelError);
        }
      } else {
        console.log("No pending orders found to cancel for this customer");
      }
    }
    ...
    cleanResponse = cleanResponse.replace(/<CUSTOMER_JSON>[\s\S]*?<\/CUSTOMER_JSON>/g, "");
    ...
    cleanResponse = cleanResponse.replace(/<CANCEL_ORDER>[\s\S]*?<\/CANCEL_ORDER>/ig, "");
    // Remove truncated/incomplete tags and everything after them
    cleanResponse = cleanResponse.replace(/<ORDER_JSON>[\s\S]*/g, "");
    cleanResponse = cleanResponse.replace(/<CUSTOMER_JSON>[\s\S]*/g, "");
    ...
    cleanResponse = cleanResponse.replace(/<CANCEL_ORDER>[\s\S]*/ig, "");
